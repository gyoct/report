...
**Page naming (Features & Model stages)**: `v<N>_<sym>_<what>.html` — version
first so same-version pages sort together. Index card labels follow
`v<N> — <what> — data <YYYYMMDD-YYYYMMDD>` (set in make_index.py OVERRIDES; the
classifier has a `^v\d+_` fallback). **Version = DATA generation** (metals:
v1 = 2026-06 window, v2 = 20260427-20260813 re-dump):
the calibrated model gen (artifact dirs *_v3, VER arg v3) runs on the same v2
window, so its pages are v2_<sym>_{model,alpha_decay}_calibrated.html; the v3
page prefix is RESERVED for the next re-dump.
Production / lifecycle / improvement / aupy pages stay unversioned (cross-version).
...
python3 gen_pipeline_models.py btc      # -> v1_btc_model.html  (VER arg -> v<N>_...)
python3 gen_alpha_decay.py btc 1 10     # -> v1_btc_alpha_decay.html (VER arg v2 -> v2_*, v3 -> v2_*_calibrated)
...
cd /home/guanyang/work/report
python3 gen_combo_report.py btc         # -> v1_btc_signal_combo.html
```
...
`v<N>_<sym>_alpha_decay.html` (stage 2c) — same formulas, virtual trades.
