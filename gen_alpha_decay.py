...
VER = next((a.lower() for a in sys.argv[2:] if not a.isdigit()), "")
ALPHA_HORIZONS = [int(a) for a in sys.argv[2:] if a.isdigit()] or [1, 10]
GEN = "pipeline_sqrtw"
...
body = f"""{fnav}<header>
  ...
  <h1>{SYM.upper()}{f" {VER}" if VER else ""} &mdash; alpha decay: virtual-trade markout by conviction threshold</h1>
...
OUT = os.path.join(HERE, f"{SYM}_alpha_decay{('_' + VER) if VER else ''}.html")
