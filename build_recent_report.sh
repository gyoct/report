...
python "$REPORT/filter_analysis_out.py" "$PROD/analysis_out" "${SINCE%% *}" "$REC"
...
[ "$PUBLISH" = 1 ] && { echo "==> publish"; (cd "$REPORT" && python make_index.py >/dev/null && echo published); } || echo "(skipped publish)"
