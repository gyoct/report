...
MODELS = [("lasso_ols", "Linear (LASSO→WLS)", "#818cf8", "lasso_ols", "pipeline_sqrtw"),
          ...
          # 2026-08-28: uniform-policy generation — fixed train z + per-feature
          # integer caps (ceil q0.9995|z|, [5,10]) seeded from pre_training;
          # tunedcal Optuna params reused; ret_1 only so far
          ("lgbm_capped", "LGBM capped (uniform norm)", "#a3e635", "lgbm", "pipeline_capstats")]
if VER:
    MODELS = [(k, l, c, d, f"{g}_{VER}") for k, l, c, d, g in MODELS]
HORIZONS = [1, 10, 30, 60]
# version-PREFIXED naming (user convention): v<N>_<sym>_model.html so the
# same version's pages sort together; the unversioned generation is v1.
...
OUT = os.path.join(HERE, f"{_PV}_{SYM}_model{_SFX}.html")


def _predcol(path, h):
    """Prediction column this report treats as 'the model output': the CALIBRATED
    Y_hat when CALIB, else the RAW column (Y_hat_<h>_raw) when present."""
    if not CALIB and f"Y_hat_{h}_raw" in pd.read_csv(path, nrows=0).columns:
        return f"Y_hat_{h}_raw"
    return f"Y_hat_{h}"
...
def perf_table(path, h, key=None):
    ...
    _pc = f"Y_hat_{h}_raw" if (has_raw and not CALIB) else f"Y_hat_{h}"
    y = df["target"].to_numpy(float); p = df[_pc].to_numpy(float)
...
def scatter_fig(key, label, color, h, sample=9000):
    ...
    if not os.path.isfile(p):
        return ""
    _pc = _predcol(p, h)
    d = pd.read_csv(p, usecols=["target", _pc], engine="c").dropna()
    if _pc != f"Y_hat_{h}":
        d = d.rename(columns={_pc: f"Y_hat_{h}"})
    if len(d) < 100:
        return ""
    ...
    for h in HORIZONS:
        ...
        for key, label, color, _d, _g in MODELS:
            ...
            if not os.path.isfile(p):
                continue
            _pc = _predcol(p, h)
            d = pd.read_csv(p, usecols=["mark_ts", "target", _pc], engine="c").dropna()
            if _pc != f"Y_hat_{h}":
                d = d.rename(columns={_pc: f"Y_hat_{h}"})
...
if VER:   # v2/v3 read the EXTENDED-window dumps (/mnt/nvme2) — override the v1 blurb
    SETUPS.update({
        "xau": "dump xau_v1 (2026-04-27 &rarr; 08-13, 100% coverage, /mnt/nvme2)"
               + (" &middot; CALIBRATED predictions" if CALIB else ""),
...
body = f"""<header>
  ...
  <h1>{SYM.upper()}{f" {VER}" if VER else ""}{" (calibrated)" if CALIB else ""} &mdash; model report (unified pipeline)</h1>
