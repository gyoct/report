...
def encrypt_dir(dir_: str) -> None:
    ...
    # A page whose password is the sentinel "public" (or "-"/"none") is left as
    # PLAINTEXT -- no password wall. If it is currently encrypted, decrypt it back.
    PUBLIC = {"public", "-", "none"}
    real_pws = [p for p in ([default_pw] + list(per_page.values()))
                if p.strip().lower() not in PUBLIC]
    done, skipped, failed, public = [], 0, [], []
    for path in sorted(glob.glob(os.path.join(dir_, "*.html"))):
        name = os.path.basename(path)
        pw = per_page.get(name, default_pw)
        raw0 = open(path, "rb").read()
        head0 = raw0[:200].decode("utf-8", "ignore")
        if pw.strip().lower() in PUBLIC:
            if head0.startswith(MARKER_PREFIX):        # was encrypted -> restore plaintext
                plain = decrypt_wrapper(raw0.decode("utf-8", "ignore"),
                                        [_derive(p, salt, cache) for p in real_pws])
                if plain is None:
                    failed.append(name); continue
                with open(path, "wb") as f:
                    f.write(plain)
                public.append(name)
            continue                                    # leave plaintext, never encrypt
    ...
    print(f"[pageenc] encrypted {len(done)} page(s) ({skipped} already current): "
          + ", ".join(done))
    if public:
        print("[pageenc] left PUBLIC (plaintext): " + ", ".join(public))
    if failed:
