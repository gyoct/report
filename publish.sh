...
( bash "$PROD/btc_monetization/rsync_bybprod.sh"
  ...
  bash "$PROD/btc_monetization_lgbm/rsync_bybprod.sh"
...
products=(btc_monetization btc_monetization2 btc_monetization3 btc_monetization_lgbm eth bgb/btc_taker)
rebuild_today=""
if [ "$(date -u +%H)" = "00" ]; then
...
declare -A REPORT_OF=(
  ...
  [btc_monetization_lgbm]=btc_prod_lgbm.html
  [eth]=eth_prod.html
...
python "$REPORT_DIR/scripts/make_index.py" --dir "$REPORT_DIR"
